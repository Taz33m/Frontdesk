    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                '/Users/tazeemmahashin/Downloads/Deskv1/mails_widget/credentials.json', SCOPES)
...
def get_calendar_events_for_date(date_str=None, timezone='UTC'):
    """Get calendar events for a specific date in a specific timezone (defaults to today in UTC)."""
    try:
        if not calendar_service:
        ...
        # Parse timezone using the existing function that handles both formats
        ...
        if date_str:
            ...
            # Use today in the specified timezone
            target_date = datetime.now(tz).date()
        
        # Create time range for the entire day in the specified timezone
        ...
        print(f"🔍 Searching events for {target_date} in {timezone}")
        print(f"   Local range: {start_of_day_local} to {end_of_day_local}")
        print(f"   UTC range: {start_of_day_utc} to {end_of_day_utc}")
...
def get_calendar_events():
    ...
    Supports both timezone formats:
    - IANA names: America/New_York, Europe/London, Asia/Tokyo
    - UTC offsets: UTC-6, UTC+2, UTC-5
    """
    try:
        ...
        timezone = request.args.get('timezone', 'UTC')  # Optional: timezone name
        
        # Get events
        events, error = get_calendar_events_for_date(date_str, timezone)
        ...
        return jsonify({
            ...
            'timezone_note': 'All event times are converted to UTC. Day boundaries calculated using specified timezone.'
        })
...
def main():
    ...
    try:
        ...
        print(f"   GET  http://localhost:{CONFIG['api_server_port']}/get_calendar_events?date=YYYY-MM-DD&timezone=TZ")
        ...
        print(f'   curl http://localhost:{CONFIG["api_server_port"]}/get_calendar_events  # Today in UTC')
        print(f'   curl http://localhost:{CONFIG["api_server_port"]}/get_calendar_events?date=2024-01-15  # Specific date in UTC')
        print(f'   curl http://localhost:{CONFIG["api_server_port"]}/get_calendar_events?timezone=America/New_York  # Today in EST')
        print(f'   curl http://localhost:{CONFIG["api_server_port"]}/get_calendar_events?timezone=UTC-6  # Today in CST (offset format)')
        print(f'   curl http://localhost:{CONFIG["api_server_port"]}/get_calendar_events?date=2024-01-15&timezone=America/Los_Angeles  # Specific date in PST')
        print(f'   curl http://localhost:{CONFIG["api_server_port"]}/get_calendar_events?date=2024-01-15&timezone=UTC+2  # Specific date UTC+2')
        print()
        print("💡 Timezone formats supported: America/New_York, UTC-6, UTC+2, Europe/London, etc.")
